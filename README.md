...
So far, we have built and verified both builtin and custom Kubernetes controllers using Welder: three controllers for managing builtin Kubernetes workloads, including ReplicaSet, Deployment, and StatefulSet, one custom controller for managing RabbitMQ deployed on Kubernetes, and a pair of controllers that mirror a custom resource into a second cluster (`widget_sync_controller/`, see `doc/widget_sync_design.md`). We used the [upstream Kubernetes controllers](https://github.com/kubernetes/kubernetes/tree/master/pkg/controller) and the [official RabbitMQ operator](https://github.com/rabbitmq/cluster-operator) as references when building our controllers. Welder is now merged into Anvil's main branch, and we are using it to build and verify more controllers.
