...
// (doc/widget_sync_fanout_design.md, section 2), against the two kind clusters
// tools/two-cluster-test.sh creates. The controller is deployed with two
// `--kind` flags, `anvil.dev/v1/Widget:field:spec.clusterName` and
...
//
// The Gadget of check 1 is named `inner`, the cluster of the single binding
// this binary is built for. Issue #40 renames that binding to `a`, and this
// name follows it.
//
...
// The cluster of the one binding the controller is built for: an object of a
// `name`-selected kind reaches the inner cluster exactly when it carries this
// name. Issue #40 renames the binding to `a`.
const BOUND_CLUSTER: &str = "inner";
// A cluster name no binding of this process has. The shim answers every request
// tagged with an unbound cluster with a Timeout, without a network round trip
// (controller_runtime::unbound_cluster_error), which the sync reconciler reports
