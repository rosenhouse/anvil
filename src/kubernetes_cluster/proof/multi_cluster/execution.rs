...
// The conjuncts of inv at one position of a simulated execution. inv is opaque,
// so a lemma that reads one of them at one state asks for it here instead of
// revealing inv over every state of the execution.
pub proof fn lemma_inv_parts_at<S>(tc: MultiCluster<S>, r: Relabeling<S>, ex: Execution<MultiClusterState<S>>, i: nat)
    requires simulation(tc, r, ex),
    ensures
        stores_sided(tc, state_at(ex, i)),
        stores_keyed(tc, state_at(ex, i)),
        msgs_ok(tc, state_at(ex, i)),
        controllers_present(tc, state_at(ex, i)),
        crs_ok(tc, state_at(ex, i)),
{
    reveal(inv);
    lemma_stores_sided_keyed(tc, state_at(ex, i));
}
...
// The multi-store step at i, relabeled, is the abstract step at i.
