...
pub open spec fn desired_outer_status(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, status: SyncedStatusView, outer: SyncedObjectView, settled: SyncedStatusView) -> bool {
    status == outer_status_for(outer.metadata.generation, settled, SyncOutcomeView::Synced)
}

pub open spec fn stored_outer_status_desired(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, outer: SyncedObjectView, settled: SyncedStatusView) -> StatePred<ClusterState> {
    |s: ClusterState| {
        ...
        &&& desired_outer_status(k, b, bs, spec_ok, stored.status->0, outer, settled)
    }
}

pub open spec fn r2_premise(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, outer: SyncedObjectView, settled: SyncedStatusView) -> TempPred<ClusterState> {
...
pub proof fn lemma_written_status_is_desired(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, outer: SyncedObjectView, settled: SyncedStatusView, inner_status: SyncedStatusView)
    requires
        bs.contains(b),
    ...
    ensures desired_outer_status(k, b, bs, spec_ok, outer_status_for(outer.metadata.generation, inner_status, SyncOutcomeView::Synced), outer, settled),
{
...
pub open spec fn r2_spec_with_premise(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView) -> TempPred<ClusterState> {
    sync_stable_spec(k, b, bs, spec_ok, cluster, controller_id, janitor_id).and(r2_premise(k, b, bs, spec_ok, outer, settled))
}

pub proof fn r2_spec_with_premise_is_stable(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        b == binding_of(k, outer),
    ensures valid(stable(r2_spec_with_premise(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled))),
{
    ...
    stable_and_n!(sync_stable_spec(k, b, bs, spec_ok, cluster, controller_id, janitor_id), r2_premise(k, b, bs, spec_ok, outer, settled));
}

pub open spec fn r2_spec_with_phase_i(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView) -> TempPred<ClusterState> {
    r2_spec_with_premise(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled).and(always(lift_state(phase_i(k, b, bs, spec_ok, controller_id))))
}

pub proof fn r2_spec_with_phase_i_is_stable(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        b == binding_of(k, outer),
    ensures valid(stable(r2_spec_with_phase_i(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled))),
{
    r2_spec_with_premise_is_stable(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled);
    always_p_is_stable(lift_state(phase_i(k, b, bs, spec_ok, controller_id)));
    stable_and_n!(r2_spec_with_premise(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled), always(lift_state(phase_i(k, b, bs, spec_ok, controller_id))));
}

pub open spec fn r2_spec_with_phase_ii(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView) -> TempPred<ClusterState> {
    r2_spec_with_phase_i(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled).and(always(lift_state(sync_phase_ii(k, b, bs, spec_ok, controller_id, outer))))
}

pub proof fn r2_spec_with_phase_ii_is_stable(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        b == binding_of(k, outer),
    ensures valid(stable(r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled))),
{
    r2_spec_with_phase_i_is_stable(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled);
    always_p_is_stable(lift_state(sync_phase_ii(k, b, bs, spec_ok, controller_id, outer)));
    stable_and_n!(r2_spec_with_phase_i(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled), always(lift_state(sync_phase_ii(k, b, bs, spec_ok, controller_id, outer))));
}

// The bridge to R1's layers: the premise of R2 gives the premise of R1 and a
// settled mirror.
pub proof fn lemma_r2_layers_imply_r1_layers(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        b == binding_of(k, outer),
        spec.entails(r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
    ensures
...
{
    entails_and_split(spec, r2_spec_with_phase_i(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled), always(lift_state(sync_phase_ii(k, b, bs, spec_ok, controller_id, outer))));
    entails_and_split(spec, r2_spec_with_premise(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled), always(lift_state(phase_i(k, b, bs, spec_ok, controller_id))));
    entails_and_split(spec, sync_stable_spec(k, b, bs, spec_ok, cluster, controller_id, janitor_id), r2_premise(k, b, bs, spec_ok, outer, settled));
    ...
    always_weaken(spec, lift_state(inner_settled(k, outer, settled)), lift_state(mirror_settled(k, b, bs, spec_ok, outer)));
    always_weaken(spec, lift_state(outer_stable(k, b, outer)), lift_state(outer_spec_stable(k, b, outer)));
    entails_and(spec, sync_stable_spec(k, b, bs, spec_ok, cluster, controller_id, janitor_id), always(lift_state(outer_spec_stable(k, b, outer))));
    entails_and(spec, sync_spec_with_desired(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer), always(lift_state(phase_i(k, b, bs, spec_ok, controller_id))));
    entails_and(spec, sync_spec_with_phase_i(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer), always(lift_state(sync_phase_ii(k, b, bs, spec_ok, controller_id, outer))));
    entails_and(spec, sync_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer), always(lift_state(mirror_settled(k, b, bs, spec_ok, outer))));
}
...
pub proof fn lemma_outer_object_facts(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, s: ClusterState, outer: SyncedObjectView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
...
pub open spec fn desired_status_patch(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, req: PatchStatusRequest, outer: SyncedObjectView, settled: SyncedStatusView) -> bool {
    ...
    &&& status is Ok
    &&& status->Ok_0 is Some
    &&& desired_outer_status(k, b, bs, spec_ok, status->Ok_0->0, outer, settled)
}
...
pub open spec fn sync_reports_no_error(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, controller_id: int, outer: SyncedObjectView) -> StatePred<ClusterState> {
    |s: ClusterState| {
        let key = outer.object_ref();
...
pub open spec fn status_writes_are_desired(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, controller_id: int, outer: SyncedObjectView, settled: SyncedStatusView) -> StatePred<ClusterState> {
    |s: ClusterState| {
        &&& sync_reports_no_error(k, b, bs, spec_ok, controller_id, outer)(s)
        &&& forall |msg: Message| {
        ...
        } ==> desired_status_patch(k, b, bs, spec_ok, msg.content.get_patch_status_request(), outer, settled)
    }
}

// The outer copy's status after one step: unchanged, or the desired one.
pub proof fn lemma_outer_status_after_step(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, 
    ...
    requires
        ...
        status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled)(s),
    ensures
        s_prime.resources()[outer.object_ref()].status == s.resources()[outer.object_ref()].status
            || stored_outer_status_desired(k, b, bs, spec_ok, outer, settled)(s_prime),
{
    let key = outer.object_ref();
    lemma_outer_object_facts(k, b, bs, spec_ok, cluster, s, outer);
    lemma_outer_object_facts(k, b, bs, spec_ok, cluster, s_prime, outer);
                        ...
                        match msg.src {
                            HostId::Controller(id, okey) => {
                                ...
                                if id == controller_id {
                                    ...
                                    assert(okey == key);
                                    assert(desired_status_patch(k, b, bs, spec_ok, req, outer, settled));
                                    if s_prime.api_server != s.api_server {
                                        assert(s_prime.resources()[key].status == req.status);
                                        assert(stored_outer_status_desired(k, b, bs, spec_ok, outer, settled)(s_prime));
                                    }
...
pub open spec fn scheduled_generation_ok(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, controller_id: int, outer: SyncedObjectView) -> StatePred<ClusterState> {
    |s: ClusterState| {
...
pub open spec fn ongoing_generation_ok(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, controller_id: int, outer: SyncedObjectView) -> StatePred<ClusterState> {
    |s: ClusterState| {
...
pub open spec fn settled_response_obj(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, obj: DynamicObjectView, outer: SyncedObjectView, settled: SyncedStatusView) -> bool {
...
pub open spec fn get_responses_are_settled(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, controller_id: int, outer: SyncedObjectView, settled: SyncedStatusView) -> StatePred<ClusterState> {
    |s: ClusterState| {
        let key = outer.object_ref();
        ...
        ==> forall |resp: Message| {
            ...
            &&& settled_response_obj(k, b, bs, spec_ok, resp.content.get_get_response().res->Ok_0, outer, settled)
        }
    }
}

pub open spec fn scheduled_status_current(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, controller_id: int, outer: SyncedObjectView, settled: SyncedStatusView) -> StatePred<ClusterState> {
    |s: ClusterState| {
        let key = outer.object_ref();
        s.scheduled_reconciles(controller_id).contains_key(key)
            ==> s.scheduled_reconciles(controller_id)[key].status == s.resources()[key].status
                || stored_outer_status_desired(k, b, bs, spec_ok, outer, settled)(s)
    }
}

pub open spec fn ongoing_status_current(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, controller_id: int, outer: SyncedObjectView, settled: SyncedStatusView) -> StatePred<ClusterState> {
    |s: ClusterState| {
        let key = outer.object_ref();
        s.ongoing_reconciles(controller_id).contains_key(key)
...
pub open spec fn r2_phase_iii(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, controller_id: int, outer: SyncedObjectView, settled: SyncedStatusView) -> StatePred<ClusterState> {
    |s: ClusterState| {
        &&& scheduled_generation_ok(k, b, bs, spec_ok, controller_id, outer)(s)
        &&& ongoing_generation_ok(k, b, bs, spec_ok, controller_id, outer)(s)
        &&& get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled)(s)
        &&& status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled)(s)
        &&& scheduled_status_current(k, b, bs, spec_ok, controller_id, outer, settled)(s)
        &&& ongoing_status_current(k, b, bs, spec_ok, controller_id, outer, settled)(s)
    }
}

pub open spec fn r2_spec_with_phase_iii(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView) -> TempPred<ClusterState> {
    r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled).and(always(lift_state(r2_phase_iii(k, b, bs, spec_ok, controller_id, outer, settled))))
}

// The action under which the phase-III facts are preserved.
pub open spec fn r2_step_next(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView) -> ActionPred<ClusterState> {
...
pub proof fn lemma_always_r2_step_next(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        ...
        sync_membership(k, b, bs, spec_ok, cluster, controller_id, janitor_id),
        spec.entails(r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
    ensures spec.entails(always(lift_action(r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)))),
{
    lemma_r2_layers_imply_r1_layers(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    ...
    combine_spec_entails_always_n!(
        spec, lift_action(r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
...
pub proof fn lemma_true_leads_to_always_scheduled_generation_ok(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        ...
        sync_membership(k, b, bs, spec_ok, cluster, controller_id, janitor_id),
        spec.entails(r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
    ensures spec.entails(true_pred().leads_to(always(lift_state(scheduled_generation_ok(k, b, bs, spec_ok, controller_id, outer))))),
{
    let key = outer.object_ref();
    lemma_r2_layers_imply_r1_layers(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_sync_stable_spec_facts(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id);
    lemma_always_r2_step_next(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    
    let next = r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled);
    let post = scheduled_generation_ok(k, b, bs, spec_ok, controller_id, outer);
...
pub proof fn lemma_true_leads_to_always_ongoing_generation_ok(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        ...
        sync_membership(k, b, bs, spec_ok, cluster, controller_id, janitor_id),
        spec.entails(r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
        spec.entails(always(lift_state(scheduled_generation_ok(k, b, bs, spec_ok, controller_id, outer)))),
    ensures spec.entails(true_pred().leads_to(always(lift_state(ongoing_generation_ok(k, b, bs, spec_ok, controller_id, outer))))),
{
    let key = outer.object_ref();
    lemma_r2_layers_imply_r1_layers(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_unfold_sync_spec_with_phase_ii(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer);
    ...
    let post = ongoing_generation_ok(k, b, bs, spec_ok, controller_id, outer);
    let next = |s, s_prime: ClusterState| {
        &&& r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)(s, s_prime)
        &&& scheduled_generation_ok(k, b, bs, spec_ok, controller_id, outer)(s)
    };
    combine_spec_entails_always_n!(
        spec, lift_action(next),
        lift_action(r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
        lift_state(scheduled_generation_ok(k, b, bs, spec_ok, controller_id, outer))
    );
...
pub proof fn lemma_get_responses_are_settled_preserved(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, 
    cluster: Cluster, controller_id: int, janitor_id: int, s: ClusterState, s_prime: ClusterState, outer: SyncedObjectView, settled: SyncedStatusView
)
    requires
        bs.contains(b),
        ...
        r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)(s, s_prime),
        get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled)(s),
    ensures get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled)(s_prime),
{
    let key = outer.object_ref();
                ...
                match step {
                    Step::APIServerStep(input) => {
                        ...
                        if !s.in_flight().contains(resp) {
                            ...
                            assert(settled_response_obj(k, b, bs, spec_ok, s.resources()[ikey], outer, settled));
                        }
                    },
...
pub proof fn lemma_true_leads_to_always_get_responses_are_settled(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        ...
        sync_membership(k, b, bs, spec_ok, cluster, controller_id, janitor_id),
        spec.entails(r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
    ensures spec.entails(true_pred().leads_to(always(lift_state(get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled))))),
{
    let key = outer.object_ref();
    lemma_r2_layers_imply_r1_layers(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_unfold_sync_spec_with_phase_ii(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer);
    ...
    lemma_always_r2_step_next(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_sync_terminates(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, key);
    let post = get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled);
    let next = r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled);
    entails_implies_leads_to(spec, lift_state(Cluster::reconcile_idle(controller_id, key)), lift_state(post));
    leads_to_trans(spec, true_pred(), lift_state(Cluster::reconcile_idle(controller_id, key)), lift_state(post));
    assert forall |s, s_prime: ClusterState| post(s) && #[trigger] next(s, s_prime) implies post(s_prime) by {
        lemma_get_responses_are_settled_preserved(k, b, bs, spec_ok, cluster, controller_id, janitor_id, s, s_prime, outer, settled);
    }
    leads_to_stable(spec, lift_action(next), true_pred(), lift_state(post));
}
...
proof fn lemma_r2_reconcile_step(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, 
    cluster: Cluster, controller_id: int, janitor_id: int, s: ClusterState, s_prime: ClusterState, outer: SyncedObjectView, settled: SyncedStatusView,
    ...
    ensures
        s_prime.ongoing_reconciles(controller_id).contains_key(outer.object_ref()) ==> ({
            ...
            &&& reconcile_prime.pending_req_msg is Some && reconcile_prime.pending_req_msg->0.content.is_patch_status_request()
                ==> desired_status_patch(k, b, bs, spec_ok, reconcile_prime.pending_req_msg->0.content.get_patch_status_request(), outer, settled)
        }),
{
    let key = outer.object_ref();
    unmarshal_of_marshal();
        ...
        match state.reconcile_step {
            ...
            WidgetSyncStepView::AfterGetInner => {
                if reconcile.pending_req_msg is None {
                    ...
                    assert(res is Ok);
                    let obj = res->Ok_0;
                    assert(settled_response_obj(k, b, bs, spec_ok, obj, outer, settled));
                    let inner = unmarshal(inner_kind(k, b), obj)->Ok_0;
                    ...
                    lemma_written_status_is_desired(k, b, bs, spec_ok, outer, settled, inner.status->0);
...
pub proof fn lemma_status_writes_are_desired_preserved(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, 
    cluster: Cluster, controller_id: int, janitor_id: int, s: ClusterState, s_prime: ClusterState, outer: SyncedObjectView, settled: SyncedStatusView
)
    requires
        bs.contains(b),
        ...
        get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled)(s),
        status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled)(s),
    ensures status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled)(s_prime),
{
    let key = outer.object_ref();
    unmarshal_of_marshal();
    WidgetSyncReconcileState::marshal_preserves_integrity();
    ...
    assert(sync_reports_no_error(k, b, bs, spec_ok, controller_id, outer)(s_prime)) by {
        if s_prime.ongoing_reconciles(controller_id).contains_key(key) {
            match step {
                Step::ControllerStep(input) => {
                    if input.0 == controller_id && input.2 == Some(key) {
                        if s.ongoing_reconciles(controller_id).contains_key(key) {
                            lemma_r2_reconcile_step(k, b, bs, spec_ok, cluster, controller_id, janitor_id, s, s_prime, outer, settled, input);
                        } else {
    ...
    assert forall |msg: Message| {
        ...
        &&& msg.content is APIRequest
        &&& msg.content.is_patch_status_request()
    } implies desired_status_patch(k, b, bs, spec_ok, msg.content.get_patch_status_request(), outer, settled) by {
...
pub proof fn lemma_true_leads_to_always_status_writes_are_desired(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        ...
        sync_membership(k, b, bs, spec_ok, cluster, controller_id, janitor_id),
        spec.entails(r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
        spec.entails(always(lift_state(ongoing_generation_ok(k, b, bs, spec_ok, controller_id, outer)))),
        spec.entails(always(lift_state(get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled)))),
    ensures spec.entails(true_pred().leads_to(always(lift_state(status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled))))),
{
    let key = outer.object_ref();
    lemma_r2_layers_imply_r1_layers(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_unfold_sync_spec_with_phase_ii(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer);
    ...
    lemma_always_r2_step_next(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_sync_terminates(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, key);
    let post = status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled);
    let next = |s, s_prime: ClusterState| {
        &&& r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)(s, s_prime)
        &&& ongoing_generation_ok(k, b, bs, spec_ok, controller_id, outer)(s)
        &&& get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled)(s)
    ...
    combine_spec_entails_always_n!(
        spec, lift_action(next),
        lift_action(r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
        lift_state(ongoing_generation_ok(k, b, bs, spec_ok, controller_id, outer)),
        lift_state(get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled))
    );
    ...
    assert forall |s, s_prime: ClusterState| post(s) && #[trigger] next(s, s_prime) implies post(s_prime) by {
        lemma_status_writes_are_desired_preserved(k, b, bs, spec_ok, cluster, controller_id, janitor_id, s, s_prime, outer, settled);
    }
    leads_to_stable(spec, lift_action(next), true_pred(), lift_state(post));
}

// (d) Snapshots' status is the stored status, unless the stored status is desired.
pub proof fn lemma_true_leads_to_always_scheduled_status_current(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        ...
        sync_membership(k, b, bs, spec_ok, cluster, controller_id, janitor_id),
        spec.entails(r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
        spec.entails(always(lift_state(status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled)))),
    ensures spec.entails(true_pred().leads_to(always(lift_state(scheduled_status_current(k, b, bs, spec_ok, controller_id, outer, settled))))),
{
    let key = outer.object_ref();
    lemma_r2_layers_imply_r1_layers(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_sync_stable_spec_facts(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id);
    ...
    let post = scheduled_status_current(k, b, bs, spec_ok, controller_id, outer, settled);
    let pre = |s: ClusterState| !post(s);
    let next = |s, s_prime: ClusterState| {
        &&& r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)(s, s_prime)
        &&& status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled)(s)
    ...
    combine_spec_entails_always_n!(
        spec, lift_action(next),
        lift_action(r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
        lift_state(status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled))
    );
    let d = outer_stable(k, b, outer);
    let stronger_pre = |s: ClusterState| pre(s) && d(s);
    assert forall |s, s_prime: ClusterState| stronger_pre(s) && #[trigger] next(s, s_prime)
    ...
    temp_pred_equality(lift_state(pre).or(lift_state(post)), true_pred());
    assert forall |s, s_prime: ClusterState| post(s) && #[trigger] next(s, s_prime) implies post(s_prime) by {
        lemma_outer_status_after_step(k, b, bs, spec_ok, cluster, controller_id, janitor_id, s, s_prime, outer, settled);
        if s_prime.scheduled_reconciles(controller_id).contains_key(key) {
            let step = choose |step| cluster.next_step(s, s_prime, step);
            match step {
                Step::ScheduleControllerReconcileStep(input) => {
...
pub proof fn lemma_true_leads_to_always_ongoing_status_current(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        ...
        spec.entails(r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
        spec.entails(always(lift_state(status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled)))),
        spec.entails(always(lift_state(scheduled_status_current(k, b, bs, spec_ok, controller_id, outer, settled)))),
    ensures spec.entails(true_pred().leads_to(always(lift_state(ongoing_status_current(k, b, bs, spec_ok, controller_id, outer, settled))))),
{
    let key = outer.object_ref();
    lemma_r2_layers_imply_r1_layers(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_unfold_sync_spec_with_phase_ii(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer);
    ...
    lemma_always_r2_step_next(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_sync_terminates(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, key);
    let post = ongoing_status_current(k, b, bs, spec_ok, controller_id, outer, settled);
    let next = |s, s_prime: ClusterState| {
        &&& r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)(s, s_prime)
        &&& status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled)(s)
        &&& scheduled_status_current(k, b, bs, spec_ok, controller_id, outer, settled)(s)
    ...
    combine_spec_entails_always_n!(
        ...
        lift_action(r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
        lift_state(status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled)),
        lift_state(scheduled_status_current(k, b, bs, spec_ok, controller_id, outer, settled))
    );
    entails_implies_leads_to(spec, lift_state(Cluster::reconcile_idle(controller_id, key)), lift_state(post));
    leads_to_trans(spec, true_pred(), lift_state(Cluster::reconcile_idle(controller_id, key)), lift_state(post));
    assert forall |s, s_prime: ClusterState| post(s) && #[trigger] next(s, s_prime) implies post(s_prime) by {
...
pub proof fn lemma_true_leads_to_always_r2_phase_iii(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        ...
        sync_membership(k, b, bs, spec_ok, cluster, controller_id, janitor_id),
        spec.entails(r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
    ensures spec.entails(true_pred().leads_to(always(lift_state(r2_phase_iii(k, b, bs, spec_ok, controller_id, outer, settled))))),
{
    let spec_ii = r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled);
    let a1 = lift_state(scheduled_generation_ok(k, b, bs, spec_ok, controller_id, outer));
    let a2 = lift_state(ongoing_generation_ok(k, b, bs, spec_ok, controller_id, outer));
    let c = lift_state(get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled));
    let bw = lift_state(status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled));
    let d1 = lift_state(scheduled_status_current(k, b, bs, spec_ok, controller_id, outer, settled));
    let d2 = lift_state(ongoing_status_current(k, b, bs, spec_ok, controller_id, outer, settled));
    r2_spec_with_phase_ii_is_stable(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled);
    assert(spec_ii.entails(spec_ii));

    // Layer 1: a1 and c directly.
    lemma_true_leads_to_always_scheduled_generation_ok(k, b, bs, spec_ok, spec_ii, cluster, controller_id, janitor_id, outer, settled);
    lemma_true_leads_to_always_get_responses_are_settled(k, b, bs, spec_ok, spec_ii, cluster, controller_id, janitor_id, outer, settled);
    ...
    lemma_true_leads_to_always_ongoing_generation_ok(k, b, bs, spec_ok, spec_l1, cluster, controller_id, janitor_id, outer, settled);
    ...
    lemma_true_leads_to_always_status_writes_are_desired(k, b, bs, spec_ok, spec_l2, cluster, controller_id, janitor_id, outer, settled);
    ...
    lemma_true_leads_to_always_scheduled_status_current(k, b, bs, spec_ok, spec_l3, cluster, controller_id, janitor_id, outer, settled);
    ...
    lemma_true_leads_to_always_ongoing_status_current(k, b, bs, spec_ok, spec_l4, cluster, controller_id, janitor_id, outer, settled);
    ...
    temp_pred_equality(lift_state(r2_phase_iii(k, b, bs, spec_ok, controller_id, outer, settled)), a1.and(c).and(a2.and(bw.and(d1.and(d2)))));
    entails_trans(spec, spec_ii, true_pred().leads_to(always(lift_state(r2_phase_iii(k, b, bs, spec_ok, controller_id, outer, settled)))));
}
...
pub proof fn lemma_unfold_r2_spec_with_phase_iii(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
        b == binding_of(k, outer),
        spec.entails(r2_spec_with_phase_iii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
    ensures
        spec.entails(r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
        spec.entails(always(lift_state(r2_phase_iii(k, b, bs, spec_ok, controller_id, outer, settled)))),
        spec.entails(always(lift_state(scheduled_generation_ok(k, b, bs, spec_ok, controller_id, outer)))),
        spec.entails(always(lift_state(ongoing_generation_ok(k, b, bs, spec_ok, controller_id, outer)))),
        spec.entails(always(lift_state(get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled)))),
        spec.entails(always(lift_state(status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled)))),
        spec.entails(always(lift_state(scheduled_status_current(k, b, bs, spec_ok, controller_id, outer, settled)))),
        spec.entails(always(lift_state(ongoing_status_current(k, b, bs, spec_ok, controller_id, outer, settled)))),
{
    entails_and_split(spec, r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled), always(lift_state(r2_phase_iii(k, b, bs, spec_ok, controller_id, outer, settled))));
    let p = lift_state(r2_phase_iii(k, b, bs, spec_ok, controller_id, outer, settled));
    always_weaken(spec, p, lift_state(scheduled_generation_ok(k, b, bs, spec_ok, controller_id, outer)));
    always_weaken(spec, p, lift_state(ongoing_generation_ok(k, b, bs, spec_ok, controller_id, outer)));
    always_weaken(spec, p, lift_state(get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled)));
    always_weaken(spec, p, lift_state(status_writes_are_desired(k, b, bs, spec_ok, controller_id, outer, settled)));
    always_weaken(spec, p, lift_state(scheduled_status_current(k, b, bs, spec_ok, controller_id, outer, settled)));
    always_weaken(spec, p, lift_state(ongoing_status_current(k, b, bs, spec_ok, controller_id, outer, settled)));
}

pub open spec fn st_status_patch_msg_in_flight(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, controller_id: int, outer: SyncedObjectView, settled: SyncedStatusView, msg: Message) -> StatePred<ClusterState> {
    |s: ClusterState| {
...
pub open spec fn st_status_patch_in_flight(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, controller_id: int, outer: SyncedObjectView, settled: SyncedStatusView) -> StatePred<ClusterState> {
    |s: ClusterState| exists |msg: Message| #[trigger] st_status_patch_msg_in_flight(k, b, bs, spec_ok, controller_id, outer, settled, msg)(s)
}

// status_synced from its parts.
pub proof fn lemma_status_synced_from_parts(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, s: ClusterState, outer: SyncedObjectView, settled: SyncedStatusView)
    requires
        bs.contains(b),
        outer.kind == k.outer_kind,
        cluster_of(k.selector, outer) is Some,
...
proof fn lemma_r2_get_resp_handled(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, 
    ...
    ensures st_status_patch_in_flight(k, b, bs, spec_ok, controller_id, outer, settled)(s_prime) || status_synced(k, outer, settled)(s_prime),
{
    let key = outer.object_ref();
    let ikey = inner_key(k, outer);
    ...
    let post = |s: ClusterState| {
        ||| st_status_patch_in_flight(k, b, bs, spec_ok, controller_id, outer, settled)(s)
    ...
    if cr_outer.status == Some(status) {
        ...
        if s.resources()[key].status == cr.status {
            ...
            assert(stored_outer_status_desired(k, b, bs, spec_ok, outer, settled)(s));
        }
        assert(stored_outer_status_desired(k, b, bs, spec_ok, outer, settled)(s_prime));
        lemma_status_synced_from_parts(k, b, bs, spec_ok, cluster, s_prime, outer, settled);
    } else {
        assert(state_prime.reconcile_step is AfterPatchOuterStatus);
        ...
        assert(desired_status_patch(k, b, bs, spec_ok, preq, outer, settled));
        assert(st_status_patch_msg_in_flight(k, b, bs, spec_ok, controller_id, outer, settled, msg)(s_prime));
    }
}

// Any other step keeps the Get response in flight and reflecting the store.
proof fn lemma_r2_get_resp_stays_in_flight(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, 
    cluster: Cluster, controller_id: int, janitor_id: int, s: ClusterState, s_prime: ClusterState, outer: SyncedObjectView, settled: SyncedStatusView, resp: Message
)
    requires
        bs.contains(b),
        ...
        ongoing_status_current(k, b, bs, spec_ok, controller_id, outer, settled)(s),
        st_get_resp_msg_in_flight(k, b, bs, spec_ok, controller_id, outer, resp)(s),
    ensures st_get_resp_msg_in_flight(k, b, bs, spec_ok, controller_id, outer, resp)(s_prime) || st_status_patch_in_flight(k, b, bs, spec_ok, controller_id, outer, settled)(s_prime) || status_synced(k, outer, settled)(s_prime),
{
    let key = outer.object_ref();
    let ikey = inner_key(k, outer);
    let input = (Some(resp), Some(key));
    ...
    match step {
        Step::ControllerStep(i) => {
            if i.0 == controller_id && i.2 == Some(key) {
                ...
                lemma_r2_get_resp_handled(k, b, bs, spec_ok, cluster, controller_id, janitor_id, s, s_prime, outer, settled, resp);
            } else {
                assert(s_prime.ongoing_reconciles(controller_id)[key] == s.ongoing_reconciles(controller_id)[key]);
...
pub proof fn lemma_r2_get_resp_leads_to_decision(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, 
    ...
    ensures
        spec.entails(lift_state(st_get_resp_in_flight(k, b, bs, spec_ok, controller_id, outer))
            .leads_to(lift_state(st_status_patch_in_flight(k, b, bs, spec_ok, controller_id, outer, settled)).or(lift_state(status_synced(k, outer, settled))))),
{
    let key = outer.object_ref();
    lemma_unfold_r2_spec_with_phase_iii(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_r2_layers_imply_r1_layers(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_unfold_sync_spec_with_phase_ii(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer);
    lemma_sync_stable_spec_facts(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id);
    lemma_always_r2_step_next(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    ...
    combine_spec_entails_always_n!(
        ...
        lift_state(get_responses_are_settled(k, b, bs, spec_ok, controller_id, outer, settled)),
        lift_state(ongoing_generation_ok(k, b, bs, spec_ok, controller_id, outer)),
        lift_state(ongoing_status_current(k, b, bs, spec_ok, controller_id, outer, settled))
    );
    assert forall |resp: Message| spec.entails(#[trigger] pre_of(resp).leads_to(lift_state(post))) by {
        ...
        assert forall |s, s_prime: ClusterState| pre(s) && #[trigger] stronger_next(s, s_prime) implies pre(s_prime) || post(s_prime) by {
            lemma_r2_get_resp_stays_in_flight(k, b, bs, spec_ok, cluster, controller_id, janitor_id, s, s_prime, outer, settled, resp);
        }
    ...
    temp_pred_equality(
        lift_state(post),
        lift_state(st_status_patch_in_flight(k, b, bs, spec_ok, controller_id, outer, settled)).or(lift_state(status_synced(k, outer, settled)))
    );
}

// The API server handles the status patch: its tests pass on the stable outer copy,
// so the desired status is stored.
proof fn lemma_r2_status_patch_handled(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, 
    ...
    requires
        ...
        st_status_patch_msg_in_flight(k, b, bs, spec_ok, controller_id, outer, settled, msg)(s),
...
proof fn lemma_r2_status_patch_stays_in_flight(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, 
    cluster: Cluster, controller_id: int, janitor_id: int, s: ClusterState, s_prime: ClusterState, outer: SyncedObjectView, settled: SyncedStatusView, msg: Message
)
    requires
        bs.contains(b),
        ...
        Cluster::each_object_in_etcd_has_at_most_one_controller_owner()(s),
        st_status_patch_msg_in_flight(k, b, bs, spec_ok, controller_id, outer, settled, msg)(s),
    ensures st_status_patch_msg_in_flight(k, b, bs, spec_ok, controller_id, outer, settled, msg)(s_prime) || status_synced(k, outer, settled)(s_prime),
{
    let key = outer.object_ref();
    let input = Some(msg);
    ...
    unmarshal_of_marshal();
    marshal_status_preserves_integrity();
    let pre = st_status_patch_msg_in_flight(k, b, bs, spec_ok, controller_id, outer, settled, msg);
    let post = status_synced(k, outer, settled);
    let step = choose |step| cluster.next_step(s, s_prime, step);
    match step {
        Step::APIServerStep(i) => {
            if i->0 == msg {
                lemma_r2_status_patch_handled(k, b, bs, spec_ok, cluster, controller_id, janitor_id, s, s_prime, outer, settled, msg);
            } else {
                assert(s_prime.in_flight().contains(msg));
                assert(s_prime.ongoing_reconciles(controller_id)[key] == s.ongoing_reconciles(controller_id)[key]);
                assert(pre(s_prime));
...
pub proof fn lemma_r2_status_patch_leads_to_synced(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, 
    spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView
)
    requires
        bs.contains(b),
        ...
        spec.entails(r2_spec_with_phase_iii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)),
    ensures
        spec.entails(lift_state(st_status_patch_in_flight(k, b, bs, spec_ok, controller_id, outer, settled)).leads_to(lift_state(status_synced(k, outer, settled)))),
{
    let key = outer.object_ref();
    lemma_unfold_r2_spec_with_phase_iii(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_r2_layers_imply_r1_layers(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    ...
    let post = status_synced(k, outer, settled);
    let pre_of = |msg: Message| lift_state(st_status_patch_msg_in_flight(k, b, bs, spec_ok, controller_id, outer, settled, msg));
    let stronger_next = |s, s_prime: ClusterState| {
        &&& r2_step_next(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled)(s, s_prime)
    ...
    assert forall |msg: Message| spec.entails(#[trigger] pre_of(msg).leads_to(lift_state(post))) by {
        ...
        assert forall |s, s_prime: ClusterState| pre(s) && #[trigger] stronger_next(s, s_prime) implies pre(s_prime) || post(s_prime) by {
            lemma_r2_status_patch_stays_in_flight(k, b, bs, spec_ok, cluster, controller_id, janitor_id, s, s_prime, outer, settled, msg);
        }
    ...
    leads_to_exists_intro(spec, pre_of, lift_state(post));
    assert_by(tla_exists(pre_of) == lift_state(st_status_patch_in_flight(k, b, bs, spec_ok, controller_id, outer, settled)), {
        assert forall |ex| #[trigger] lift_state(st_status_patch_in_flight(k, b, bs, spec_ok, controller_id, outer, settled)).satisfied_by(ex)
        implies tla_exists(pre_of).satisfied_by(ex) by {
            let s = ex.head();
            let msg = choose |msg: Message| #[trigger] st_status_patch_msg_in_flight(k, b, bs, spec_ok, controller_id, outer, settled, msg)(s);
            assert(pre_of(msg).satisfied_by(ex));
        }
        assert forall |ex| #[trigger] tla_exists(pre_of).satisfied_by(ex)
        implies lift_state(st_status_patch_in_flight(k, b, bs, spec_ok, controller_id, outer, settled)).satisfied_by(ex) by {
            let msg = choose |msg: Message| #[trigger] pre_of(msg).satisfied_by(ex);
            assert(st_status_patch_msg_in_flight(k, b, bs, spec_ok, controller_id, outer, settled, msg)(ex.head()));
        }
        temp_pred_equality(tla_exists(pre_of), lift_state(st_status_patch_in_flight(k, b, bs, spec_ok, controller_id, outer, settled)));
    });
}

// Under all layers: the desired status is eventually and forever stored.
pub proof fn lemma_true_leads_to_always_status_synced(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, 
    spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView
)
    requires
        bs.contains(b),
    ...
    ensures spec.entails(true_pred().leads_to(always(lift_state(status_synced(k, outer, settled))))),
{
    ...
    let status_req = lift_state(st_status_patch_in_flight(k, b, bs, spec_ok, controller_id, outer, settled));
    ...
    lemma_r2_get_resp_leads_to_decision(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
    lemma_r2_status_patch_leads_to_synced(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, outer, settled);
...
pub proof fn lemma_premise_leads_to_always_status_synced(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, 
    spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView, settled: SyncedStatusView
)
    requires
        bs.contains(b),
        ...
        sync_membership(k, b, bs, spec_ok, cluster, controller_id, janitor_id),
        spec.entails(sync_stable_spec(k, b, bs, spec_ok, cluster, controller_id, janitor_id)),
    ensures spec.entails(r2_premise(k, b, bs, spec_ok, outer, settled).leads_to(always(lift_state(status_synced(k, outer, settled))))),
{
    ...
    let spec_p = r2_spec_with_premise(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled);
    let spec_i = r2_spec_with_phase_i(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled);
    let spec_ii = r2_spec_with_phase_ii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled);
    let spec_iii = r2_spec_with_phase_iii(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled);
    let phase_iii_temp = always(lift_state(r2_phase_iii(k, b, bs, spec_ok, controller_id, outer, settled)));
    ...
    let premise_temp = r2_premise(k, b, bs, spec_ok, outer, settled);

    assert(spec_iii.entails(spec_iii));
    lemma_true_leads_to_always_status_synced(k, b, bs, spec_ok, spec_iii, cluster, controller_id, janitor_id, outer, settled);
    // Remove phase III.
    r2_spec_with_phase_ii_is_stable(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer, settled);
    ...
    assert(spec_ii.entails(spec_ii));
    lemma_true_leads_to_always_r2_phase_iii(k, b, bs, spec_ok, spec_ii, cluster, controller_id, janitor_id, outer, settled);
    ...
    entails_and(spec_i, sync_spec_with_desired(k, b, bs, spec_ok, cluster, controller_id, janitor_id, outer), phase_i_temp);
    ...
    lemma_sync_stable_spec_facts(k, b, bs, spec_ok, spec_p, cluster, controller_id, janitor_id);
    lemma_true_leads_to_always_phase_i(k, b, bs, spec_ok, spec_p, cluster, controller_id);
...
pub proof fn sync_eventually_mirrors_status(k: SyncKind, b: Binding, bs: Set<Binding>, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int)
    requires
        bs.contains(b),
    ...
    ensures spec.entails(widget_status_eventually_mirrored(k, b)),
{
    ...
    assert forall |i: (SyncedObjectView, SyncedStatusView)| spec.entails(#[trigger] per_cr(i)) by {
        if i.0.kind == k.outer_kind && cluster_of(k.selector, i.0) is Some && b == binding_of(k, i.0) {
            lemma_premise_leads_to_always_status_synced(k, b, bs, spec_ok, spec, cluster, controller_id, janitor_id, i.0, i.1);
        } else {
