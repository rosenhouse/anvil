...
// widget_sync_rely.
pub open spec fn sync_rely_with_janitor(k: SyncKind, b: Binding, cluster: Cluster, controller_id: int, janitor_id: int) -> StatePred<ClusterState> {
    |s: ClusterState| {
...
{
    let rely = sync_rely_with_janitor(k, b, cluster, controller_id, janitor_id);
    ...
    assert forall |s: ClusterState| #[trigger] all(s) implies {
        ...
        assert(widget_janitor_guarantee(k, b, janitor_id)(s));
    ...
    always_weaken(spec, lift_state(all), lift_state(every_in_flight_inner_create_is_a_mirror_create(k)));
    always_weaken(spec, lift_state(all), lift_state(every_in_flight_inner_update_preserves_identity(k)));
    always_weaken(spec, lift_state(all), lift_state(widget_janitor_guarantee(k, b, janitor_id)));
}
...
pub open spec fn sync_invariants(k: SyncKind, b: Binding, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int, janitor_id: int) -> TempPred<ClusterState> {
    ...
    .and(always(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(inner_kind(k, b)))))
    .and(always(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(k.outer_kind))))
    .and(always(lift_state(cluster.synced_objects_in_reconcile_are_valid(k.outer_kind, spec_ok, controller_id))))
    ...
    .and(always(lift_state(Cluster::objects_in_reconcile_have_kind(k.outer_kind, controller_id))))
    ...
    .and(always(lift_state(Cluster::synced_states_are_unmarshallable::<WidgetSyncReconcileState>(k.outer_kind, controller_id))))
    ...
    .and(always(lift_state(widget_sync_guarantee(k, controller_id))))
    .and(always(lift_state(widget_janitor_guarantee(k, b, janitor_id))))
    ...
    ensures valid(stable(sync_invariants(k, b, spec_ok, cluster, controller_id, janitor_id))),
{
    ...
    always_p_is_stable(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(inner_kind(k, b))));
    always_p_is_stable(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(k.outer_kind)));
    always_p_is_stable(lift_state(cluster.synced_objects_in_reconcile_are_valid(k.outer_kind, spec_ok, controller_id)));
    ...
    always_p_is_stable(lift_state(Cluster::objects_in_reconcile_have_kind(k.outer_kind, controller_id)));
    ...
    always_p_is_stable(lift_state(Cluster::synced_states_are_unmarshallable::<WidgetSyncReconcileState>(k.outer_kind, controller_id)));
    ...
    always_p_is_stable(lift_state(widget_sync_guarantee(k, controller_id)));
    always_p_is_stable(lift_state(widget_janitor_guarantee(k, b, janitor_id)));
    ...
    stable_and_n!(
        ...
        always(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(inner_kind(k, b)))),
        always(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(k.outer_kind))),
        always(lift_state(cluster.synced_objects_in_reconcile_are_valid(k.outer_kind, spec_ok, controller_id))),
        ...
        always(lift_state(Cluster::objects_in_reconcile_have_kind(k.outer_kind, controller_id))),
        ...
        always(lift_state(Cluster::synced_states_are_unmarshallable::<WidgetSyncReconcileState>(k.outer_kind, controller_id))),
        ...
        always(lift_state(widget_sync_guarantee(k, controller_id))),
        always(lift_state(widget_janitor_guarantee(k, b, janitor_id))),
        ...
        spec.entails(always(lift_state(Cluster::all_requests_from_builtin_controllers_are_api_delete_requests()))),
{
    ...
    cluster.lemma_always_every_in_flight_msg_from_controller_has_key_kind(spec, k.outer_kind, controller_id);
...
pub open spec fn sync_stable_spec(k: SyncKind, b: Binding, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int, janitor_id: int) -> TempPred<ClusterState> {
    ...
    .and(always(lift_state(sync_rely_with_janitor(k, b, cluster, controller_id, janitor_id))))
    .and(inner_releases_terminating_objects(k, b))
    ...
    ensures valid(stable(sync_stable_spec(k, b, spec_ok, cluster, controller_id, janitor_id))),
{
    ...
    always_p_is_stable(lift_state(sync_rely_with_janitor(k, b, cluster, controller_id, janitor_id)));
    assert(valid(stable(inner_releases_terminating_objects(k, b)))) by {
        let p = |i: (ObjectRef, Uid)| lift_state(inner_terminating_object(k, b, i.0, i.1));
        ...
        temp_pred_equality(inner_releases_terminating_objects(k, b), tla_forall(|i: (ObjectRef, Uid)| p(i).leads_to(q(i))));
    }
        ...
        spec.entails(always(lift_state(sync_pending_requests_match_snapshots(k, controller_id)))),
{
    ...
    entails_and_split(spec, wf.and(always(lift_state(sync_rely_with_janitor(k, b, cluster, controller_id, janitor_id)))).and(inner_releases_terminating_objects(k, b)).and(widget_mirrors_eventually_collected(k, b)), inv);
    entails_and_split(spec, wf.and(always(lift_state(sync_rely_with_janitor(k, b, cluster, controller_id, janitor_id)))).and(inner_releases_terminating_objects(k, b)), widget_mirrors_eventually_collected(k, b));
    entails_and_split(spec, wf.and(always(lift_state(sync_rely_with_janitor(k, b, cluster, controller_id, janitor_id)))), inner_releases_terminating_objects(k, b));
    entails_and_split(spec, wf, always(lift_state(sync_rely_with_janitor(k, b, cluster, controller_id, janitor_id))));
    ...
    assert(inv.entails(always(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(inner_kind(k, b))))));
    entails_trans(spec, inv, always(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(inner_kind(k, b)))));
    assert(inv.entails(always(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(k.outer_kind)))));
    entails_trans(spec, inv, always(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(k.outer_kind))));
    ...
    assert(inv.entails(always(lift_state(cluster.synced_objects_in_reconcile_are_valid(k.outer_kind, spec_ok, controller_id)))));
    entails_trans(spec, inv, always(lift_state(cluster.synced_objects_in_reconcile_are_valid(k.outer_kind, spec_ok, controller_id))));
    ...
    assert(inv.entails(always(lift_state(Cluster::objects_in_reconcile_have_kind(k.outer_kind, controller_id)))));
    entails_trans(spec, inv, always(lift_state(Cluster::objects_in_reconcile_have_kind(k.outer_kind, controller_id))));
    assert(inv.entails(always(lift_state(Cluster::every_in_flight_msg_from_controller_has_key_kind(k.outer_kind, controller_id)))));
    entails_trans(spec, inv, always(lift_state(Cluster::every_in_flight_msg_from_controller_has_key_kind(k.outer_kind, controller_id))));
    ...
    assert(inv.entails(always(lift_state(Cluster::synced_states_are_unmarshallable::<WidgetSyncReconcileState>(k.outer_kind, controller_id)))));
    entails_trans(spec, inv, always(lift_state(Cluster::synced_states_are_unmarshallable::<WidgetSyncReconcileState>(k.outer_kind, controller_id))));
    ...
    assert(inv.entails(always(lift_state(widget_sync_guarantee(k, controller_id)))));
    entails_trans(spec, inv, always(lift_state(widget_sync_guarantee(k, controller_id))));
    assert(inv.entails(always(lift_state(widget_janitor_guarantee(k, b, janitor_id)))));
    entails_trans(spec, inv, always(lift_state(widget_janitor_guarantee(k, b, janitor_id))));
    assert(inv.entails(always(lift_state(every_in_flight_inner_update_preserves_identity(k)))));
    ...
    entails_trans(spec, inv, always(lift_state(janitor_deletes_are_sound(k, b, janitor_id))));
    assert(inv.entails(always(lift_state(builtin_deletes_never_target_mirrors(k, b)))));
...
// cluster's structural invariants, says about writes of mirrors by anyone.
pub proof fn lemma_janitor_rely_implies_mirror_write_facts(k: SyncKind, b: Binding, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int)
    requires
        k.bindings.contains(b),
        ...
        spec.entails(always(lift_state(every_in_flight_inner_update_preserves_identity(k)))),
{
    ...
    temp_pred_equality(
        lift_state(all),
        lift_state(rely)
            .and(lift_state(widget_janitor_guarantee(k, b, controller_id)))
            .and(lift_state(cluster.every_in_flight_req_msg_from_controller_has_valid_controller_id()))
            .and(lift_state(Cluster::no_pending_request_to_api_server_from_api_server_or_external()))
            .and(lift_state(Cluster::all_requests_from_pod_monkey_are_api_pod_requests()))
            .and(lift_state(Cluster::all_requests_from_builtin_controllers_are_api_delete_requests()))
    );
    assert forall |s: ClusterState| #[trigger] all(s) implies every_in_flight_inner_create_is_a_mirror_create(k)(s) && every_in_flight_inner_update_preserves_identity(k)(s) by {
        assert forall |msg: Message| {
            &&& #[trigger] s.in_flight().contains(msg)
            &&& msg.dst is APIServer
            &&& msg.content is APIRequest
        } implies request_is_a_sound_mirror_write(k, msg, s) by {
            match msg.src {
                HostId::Controller(id, key) => {
                    ...
                    if id == controller_id {
                        ...
                        assert(cluster.controller_models.remove(controller_id).contains_key(id));
                        assert(widget_janitor_rely(k, id)(s));
                    }
                },
                HostId::BuiltinController => {},
                HostId::PodMonkey => {},
...
pub open spec fn request_is_a_sound_mirror_write(k: SyncKind, msg: Message, s: ClusterState) -> bool {
    &&& (msg.content.is_create_request() && is_inner_kind(k, msg.content.get_create_request().obj.kind)) ==> {
        let req = msg.content.get_create_request();
        &&& req.obj.metadata.name is Some
        &&& mirror_create_req(k, req, ObjectRef {
...
pub open spec fn janitor_invariants(k: SyncKind, b: Binding, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int) -> TempPred<ClusterState> {
    ...
    .and(always(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(inner_kind(k, b)))))
    .and(always(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(k.outer_kind))))
    .and(always(lift_state(cluster.synced_objects_in_reconcile_are_valid(inner_kind(k, b), spec_ok, controller_id))))
    .and(always(lift_state(cluster.every_in_flight_req_msg_from_controller_has_valid_controller_id())))
    .and(always(lift_state(Cluster::every_in_flight_msg_has_no_replicas_and_has_unique_id())))
    .and(always(lift_state(Cluster::each_scheduled_object_has_consistent_key_and_valid_metadata(controller_id))))
    .and(always(lift_state(Cluster::each_object_in_reconcile_has_consistent_key_and_valid_metadata(controller_id))))
    .and(always(lift_state(Cluster::every_ongoing_reconcile_has_lower_id_than_allocator(controller_id))))
    .and(always(lift_state(Cluster::objects_in_reconcile_have_kind(inner_kind(k, b), controller_id))))
    .and(always(tla_forall(|key: ObjectRef| lift_state(Cluster::pending_req_of_key_is_unique_with_unique_id(controller_id, key)))))
    .and(always(lift_state(Cluster::there_is_the_controller_state(controller_id))))
    .and(always(lift_state(Cluster::there_is_no_request_msg_to_external_from_controller(controller_id))))
    .and(always(lift_state(Cluster::synced_states_are_unmarshallable::<WidgetJanitorReconcileState>(inner_kind(k, b), controller_id))))
    ...
    .and(always(tla_forall(|key: ObjectRef| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, key, cluster.reconcile_model(controller_id).done)))))
    .and(always(tla_forall(|key: ObjectRef| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, key, cluster.reconcile_model(controller_id).error)))))
    .and(always(lift_state(Cluster::every_in_flight_msg_from_controller_has_key_kind(inner_kind(k, b), controller_id))))
    .and(always(lift_state(Cluster::no_pending_request_to_api_server_from_api_server_or_external())))
    .and(always(lift_state(Cluster::all_requests_from_pod_monkey_are_api_pod_requests())))
    .and(always(lift_state(Cluster::all_requests_from_builtin_controllers_are_api_delete_requests())))
    .and(always(lift_state(widget_janitor_guarantee(k, b, controller_id))))
    .and(always(lift_state(every_in_flight_inner_create_is_a_mirror_create(k))))
    .and(always(lift_state(every_in_flight_inner_update_preserves_identity(k))))
    .and(always(lift_state(every_mirror_is_bound(k, b))))
    .and(always(lift_state(janitor_scheduled_crs_are_sound(k, b, controller_id))))
    .and(always(lift_state(janitor_triggering_crs_are_sound(k, b, controller_id))))
    ...
    ensures valid(stable(janitor_invariants(k, b, spec_ok, cluster, controller_id))),
{
    ...
    always_p_is_stable(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(inner_kind(k, b))));
    always_p_is_stable(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(k.outer_kind)));
    always_p_is_stable(lift_state(cluster.synced_objects_in_reconcile_are_valid(inner_kind(k, b), spec_ok, controller_id)));
    always_p_is_stable(lift_state(cluster.every_in_flight_req_msg_from_controller_has_valid_controller_id()));
    always_p_is_stable(lift_state(Cluster::every_in_flight_msg_has_no_replicas_and_has_unique_id()));
    always_p_is_stable(lift_state(Cluster::each_scheduled_object_has_consistent_key_and_valid_metadata(controller_id)));
    always_p_is_stable(lift_state(Cluster::each_object_in_reconcile_has_consistent_key_and_valid_metadata(controller_id)));
    always_p_is_stable(lift_state(Cluster::every_ongoing_reconcile_has_lower_id_than_allocator(controller_id)));
    always_p_is_stable(lift_state(Cluster::objects_in_reconcile_have_kind(inner_kind(k, b), controller_id)));
    always_p_is_stable(tla_forall(|key: ObjectRef| lift_state(Cluster::pending_req_of_key_is_unique_with_unique_id(controller_id, key))));
    always_p_is_stable(lift_state(Cluster::there_is_the_controller_state(controller_id)));
    always_p_is_stable(lift_state(Cluster::there_is_no_request_msg_to_external_from_controller(controller_id)));
    always_p_is_stable(lift_state(Cluster::synced_states_are_unmarshallable::<WidgetJanitorReconcileState>(inner_kind(k, b), controller_id)));
    ...
    always_p_is_stable(tla_forall(|key: ObjectRef| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, key, cluster.reconcile_model(controller_id).done))));
    always_p_is_stable(tla_forall(|key: ObjectRef| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, key, cluster.reconcile_model(controller_id).error))));
    always_p_is_stable(lift_state(Cluster::every_in_flight_msg_from_controller_has_key_kind(inner_kind(k, b), controller_id)));
    always_p_is_stable(lift_state(Cluster::no_pending_request_to_api_server_from_api_server_or_external()));
    always_p_is_stable(lift_state(Cluster::all_requests_from_pod_monkey_are_api_pod_requests()));
    always_p_is_stable(lift_state(Cluster::all_requests_from_builtin_controllers_are_api_delete_requests()));
    always_p_is_stable(lift_state(widget_janitor_guarantee(k, b, controller_id)));
    always_p_is_stable(lift_state(every_in_flight_inner_create_is_a_mirror_create(k)));
    always_p_is_stable(lift_state(every_in_flight_inner_update_preserves_identity(k)));
    always_p_is_stable(lift_state(every_mirror_is_bound(k, b)));
    always_p_is_stable(lift_state(janitor_scheduled_crs_are_sound(k, b, controller_id)));
    always_p_is_stable(lift_state(janitor_triggering_crs_are_sound(k, b, controller_id)));
    always_p_is_stable(lift_state(janitor_decisions_are_sound(k, b, controller_id)));
    stable_and_n!(
        always(lift_state(Cluster::every_in_flight_msg_has_unique_id())),
        always(lift_state(Cluster::every_in_flight_msg_has_lower_id_than_allocator())),
        always(lift_state(Cluster::every_in_flight_req_msg_has_different_id_from_pending_req_msg_of_every_ongoing_reconcile(controller_id))),
        ...
        always(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(inner_kind(k, b)))),
        always(lift_state(cluster.each_synced_object_in_etcd_is_well_formed(k.outer_kind))),
        always(lift_state(cluster.synced_objects_in_reconcile_are_valid(inner_kind(k, b), spec_ok, controller_id))),
        always(lift_state(cluster.every_in_flight_req_msg_from_controller_has_valid_controller_id())),
        always(lift_state(Cluster::every_in_flight_msg_has_no_replicas_and_has_unique_id())),
        always(lift_state(Cluster::each_scheduled_object_has_consistent_key_and_valid_metadata(controller_id))),
        always(lift_state(Cluster::each_object_in_reconcile_has_consistent_key_and_valid_metadata(controller_id))),
        always(lift_state(Cluster::every_ongoing_reconcile_has_lower_id_than_allocator(controller_id))),
        always(lift_state(Cluster::objects_in_reconcile_have_kind(inner_kind(k, b), controller_id))),
        always(tla_forall(|key: ObjectRef| lift_state(Cluster::pending_req_of_key_is_unique_with_unique_id(controller_id, key)))),
        always(lift_state(Cluster::there_is_the_controller_state(controller_id))),
        always(lift_state(Cluster::there_is_no_request_msg_to_external_from_controller(controller_id))),
        always(lift_state(Cluster::synced_states_are_unmarshallable::<WidgetJanitorReconcileState>(inner_kind(k, b), controller_id))),
        ...
        always(tla_forall(|key: ObjectRef| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, key, cluster.reconcile_model(controller_id).done)))),
        always(tla_forall(|key: ObjectRef| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, key, cluster.reconcile_model(controller_id).error)))),
        always(lift_state(Cluster::every_in_flight_msg_from_controller_has_key_kind(inner_kind(k, b), controller_id))),
        always(lift_state(Cluster::no_pending_request_to_api_server_from_api_server_or_external())),
        always(lift_state(Cluster::all_requests_from_pod_monkey_are_api_pod_requests())),
        always(lift_state(Cluster::all_requests_from_builtin_controllers_are_api_delete_requests())),
        always(lift_state(widget_janitor_guarantee(k, b, controller_id))),
        always(lift_state(every_in_flight_inner_create_is_a_mirror_create(k))),
        always(lift_state(every_in_flight_inner_update_preserves_identity(k))),
        always(lift_state(every_mirror_is_bound(k, b))),
        always(lift_state(janitor_scheduled_crs_are_sound(k, b, controller_id))),
        always(lift_state(janitor_triggering_crs_are_sound(k, b, controller_id))),
        always(lift_state(janitor_decisions_are_sound(k, b, controller_id)))
    );
}
    ...
    ensures spec.entails(janitor_invariants(k, b, spec_ok, cluster, controller_id)),
{
    ...
    cluster.lemma_always_there_is_the_controller_state(spec, controller_id);
    cluster.lemma_always_there_is_no_request_msg_to_external_from_controller(spec, controller_id);
    cluster.lemma_always_synced_states_are_unmarshallable::<WidgetJanitorReconcileState, VoidEReqView, VoidERespView>(spec, inner_kind(k, b), || janitor_reconciler::reconcile_init_state(), |obj: SyncedObjectView, resp_o, st| janitor_reconciler::reconcile_core(k, b, obj, resp_o, st), |st| janitor_reconciler::reconcile_done(st), |st| janitor_reconciler::reconcile_error(st), controller_id);
    ...
    spec_entails_always_tla_forall_equality(spec, |key: ObjectRef| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, key, cluster.reconcile_model(controller_id).error)));
    cluster.lemma_always_every_in_flight_msg_from_controller_has_key_kind(spec, inner_kind(k, b), controller_id);
    cluster.lemma_always_no_pending_request_to_api_server_from_api_server_or_external(spec);
    cluster.lemma_always_all_requests_from_pod_monkey_are_api_pod_requests(spec);
    cluster.lemma_always_all_requests_from_builtin_controllers_are_api_delete_requests(spec);
    ...
    entails_always_and_n!(
        ...
        lift_state(cluster.each_synced_object_in_etcd_is_well_formed(inner_kind(k, b))),
        lift_state(cluster.each_synced_object_in_etcd_is_well_formed(k.outer_kind)),
        lift_state(cluster.synced_objects_in_reconcile_are_valid(inner_kind(k, b), spec_ok, controller_id)),
        lift_state(cluster.every_in_flight_req_msg_from_controller_has_valid_controller_id()),
        lift_state(Cluster::every_in_flight_msg_has_no_replicas_and_has_unique_id()),
        lift_state(Cluster::each_scheduled_object_has_consistent_key_and_valid_metadata(controller_id)),
        lift_state(Cluster::each_object_in_reconcile_has_consistent_key_and_valid_metadata(controller_id)),
        ...
        tla_forall(|key: ObjectRef| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, key, cluster.reconcile_model(controller_id).done))),
        tla_forall(|key: ObjectRef| lift_state(Cluster::no_pending_req_msg_at_reconcile_state(controller_id, key, cluster.reconcile_model(controller_id).error))),
        lift_state(Cluster::every_in_flight_msg_from_controller_has_key_kind(inner_kind(k, b), controller_id)),
        lift_state(Cluster::no_pending_request_to_api_server_from_api_server_or_external()),
        lift_state(Cluster::all_requests_from_pod_monkey_are_api_pod_requests()),
        lift_state(Cluster::all_requests_from_builtin_controllers_are_api_delete_requests()),
        lift_state(widget_janitor_guarantee(k, b, controller_id)),
        ...
        lift_state(every_in_flight_inner_update_preserves_identity(k)),
        lift_state(every_mirror_is_bound(k, b)),
        lift_state(janitor_scheduled_crs_are_sound(k, b, controller_id)),
        ...
        spec.entails(always(lift_state(janitor_decisions_are_sound(k, b, controller_id)))),
{
    ...
    assert(stable_spec.entails(inner_releases_terminating_objects(k, b)));
    ...
    entails_trans(spec, stable_spec, inner_releases_terminating_objects(k, b));
    ...
    assert(inv.entails(always(lift_state(Cluster::every_ongoing_reconcile_has_lower_id_than_allocator(controller_id)))));
    entails_trans(spec, inv, always(lift_state(Cluster::every_ongoing_reconcile_has_lower_id_than_allocator(controller_id))));
    assert(inv.entails(always(lift_state(Cluster::objects_in_reconcile_have_kind(inner_kind(k, b), controller_id)))));
    entails_trans(spec, inv, always(lift_state(Cluster::objects_in_reconcile_have_kind(inner_kind(k, b), controller_id))));
    assert(inv.entails(always(tla_forall(|key: ObjectRef| lift_state(Cluster::pending_req_of_key_is_unique_with_unique_id(controller_id, key))))));
    entails_trans(spec, inv, always(tla_forall(|key: ObjectRef| lift_state(Cluster::pending_req_of_key_is_unique_with_unique_id(controller_id, key)))));
    assert(inv.entails(always(lift_state(Cluster::there_is_the_controller_state(controller_id)))));
    entails_trans(spec, inv, always(lift_state(Cluster::there_is_the_controller_state(controller_id))));
    assert(inv.entails(always(lift_state(Cluster::there_is_no_request_msg_to_external_from_controller(controller_id)))));
    entails_trans(spec, inv, always(lift_state(Cluster::there_is_no_request_msg_to_external_from_controller(controller_id))));
    assert(inv.entails(always(lift_state(Cluster::synced_states_are_unmarshallable::<WidgetJanitorReconcileState>(inner_kind(k, b), controller_id)))));
    entails_trans(spec, inv, always(lift_state(Cluster::synced_states_are_unmarshallable::<WidgetJanitorReconcileState>(inner_kind(k, b), controller_id))));
    ...
    assert(inv.entails(always(lift_state(every_mirror_is_bound(k, b)))));
    entails_trans(spec, inv, always(lift_state(every_mirror_is_bound(k, b))));
    assert(inv.entails(always(lift_state(janitor_triggering_crs_are_sound(k, b, controller_id)))));
    entails_trans(spec, inv, always(lift_state(janitor_triggering_crs_are_sound(k, b, controller_id))));
    assert(inv.entails(always(lift_state(janitor_decisions_are_sound(k, b, controller_id)))));
    entails_trans(spec, inv, always(lift_state(janitor_decisions_are_sound(k, b, controller_id))));
}

// A leads-to whose premise no state satisfies. R1, R2 and R3s are stated per
// binding and their premises fix the binding of the object they are about, so
// for an object of another binding the promise is vacuous.
pub proof fn lemma_vacuous_always_leads_to(spec: TempPred<ClusterState>, p: StatePred<ClusterState>, q: TempPred<ClusterState>)
    requires forall |s: ClusterState| !#[trigger] p(s),
    ensures spec.entails(always(lift_state(p)).leads_to(q)),
{
    assert forall |ex: Execution<ClusterState>| #[trigger] spec.satisfied_by(ex)
    implies always(lift_state(p)).leads_to(q).satisfied_by(ex) by {
        assert forall |i| #[trigger] always(lift_state(p)).satisfied_by(ex.suffix(i))
        implies eventually(q).satisfied_by(ex.suffix(i)) by {
            assert(lift_state(p).satisfied_by(ex.suffix(i).suffix(0)));
            assert(p(ex.suffix(i).suffix(0).head()));
        }
    }
}
...
pub open spec fn mirror_absent(k: SyncKind, outer: SyncedObjectView) -> StatePred<ClusterState> {
    |s: ClusterState| !s.resources().contains_key(inner_key(k, outer))
}

// The mirror of `outer` is there, is ours, and is not terminating.
pub open spec fn mirror_is_ours(k: SyncKind, b: Binding, outer: SyncedObjectView) -> StatePred<ClusterState> {
    |s: ClusterState| {
...
pub open spec fn mirror_settled(k: SyncKind, b: Binding, outer: SyncedObjectView) -> StatePred<ClusterState> {
...
pub open spec fn present_or_gone(k: SyncKind, b: Binding, key: ObjectRef, parent_uid: Uid, uid: Uid) -> StatePred<ClusterState> {
    |s: ClusterState| mirror_object_is(inner_kind(k, b), key, parent_uid, uid)(s) || gone(key, uid)(s)
}

// A snapshot of the mirror object.
pub open spec fn snapshot_of_mirror(k: SyncKind, b: Binding, cr: DynamicObjectView, key: ObjectRef, parent_uid: Uid, uid: Uid) -> bool {
    &&& snapshot_is_mirror(inner_kind(k, b), cr)
    &&& cr.object_ref() == key
    &&& cr.metadata.uid == Some(uid)
    &&& snapshot_parent(inner_kind(k, b), cr) == int_to_string_view(parent_uid)
}
...
pub proof fn lemma_true_leads_to_always_phase_i(k: SyncKind, b: Binding, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int)
    requires
        k.bindings.contains(b),
        spec.entails(always(lift_action(cluster.next()))),
        spec.entails(always(lift_state(Cluster::there_is_the_controller_state(controller_id)))),
...
pub open spec fn sync_spec_with_phase_i(k: SyncKind, b: Binding, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView) -> TempPred<ClusterState> {
    sync_spec_with_desired(k, b, spec_ok, cluster, controller_id, janitor_id, outer).and(always(lift_state(phase_i(controller_id))))
}
    ...
    ensures valid(stable(sync_spec_with_phase_i(k, b, spec_ok, cluster, controller_id, janitor_id, outer))),
{
    ...
    stable_and_n!(sync_spec_with_desired(k, b, spec_ok, cluster, controller_id, janitor_id, outer), always(lift_state(phase_i(controller_id))));
}
...
pub open spec fn sync_phase_ii(controller_id: int, outer: SyncedObjectView) -> StatePred<ClusterState> {
    |s: ClusterState| {
...
pub open spec fn sync_spec_with_phase_ii(k: SyncKind, b: Binding, spec_ok: spec_fn(Value) -> bool, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView) -> TempPred<ClusterState> {
    sync_spec_with_phase_i(k, b, spec_ok, cluster, controller_id, janitor_id, outer).and(always(lift_state(sync_phase_ii(controller_id, outer))))
}
...
pub proof fn lemma_unfold_sync_spec_with_phase_ii(k: SyncKind, b: Binding, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView)
    ...
    ensures
        ...
        spec.entails(always(lift_state(Cluster::the_synced_object_in_schedule_is(controller_id, outer)))),
        spec.entails(always(lift_state(Cluster::the_synced_object_in_reconcile_is(controller_id, outer)))),
...
{
    entails_and_split(spec, sync_spec_with_phase_i(k, b, spec_ok, cluster, controller_id, janitor_id, outer), always(lift_state(sync_phase_ii(controller_id, outer))));
    entails_and_split(spec, sync_spec_with_desired(k, b, spec_ok, cluster, controller_id, janitor_id, outer), always(lift_state(phase_i(controller_id))));
    ...
    always_weaken(spec, lift_state(sync_phase_ii(controller_id, outer)), lift_state(Cluster::the_synced_object_in_schedule_is(controller_id, outer)));
    always_weaken(spec, lift_state(sync_phase_ii(controller_id, outer)), lift_state(Cluster::the_synced_object_in_reconcile_is(controller_id, outer)));
...
// copy's spec and uid.
pub open spec fn sync_phase_ii_rest(controller_id: int, outer: SyncedObjectView) -> StatePred<ClusterState> {
    |s: ClusterState| {
        &&& Cluster::the_synced_object_in_reconcile_is(controller_id, outer)(s)
        &&& Cluster::every_msg_from_key_is_pending_req_msg_of(controller_id, outer.object_ref())(s)
        &&& Cluster::pending_req_in_flight_xor_resp_in_flight_if_has_pending_req_msg(controller_id, outer.object_ref())(s)
        ...
        spec.entails(true_pred().leads_to(lift_state(Cluster::reconcile_idle(controller_id, key)))),
{
    ...
    terminate::sync_reconcile_eventually_terminates(k, b, spec, cluster, controller_id);
    ...
    ensures spec.entails(true_pred().leads_to(always(lift_state(sync_phase_ii(controller_id, outer))))),
{
    ...
    entails_and_split(spec_i, sync_spec_with_desired(k, b, spec_ok, cluster, controller_id, janitor_id, outer), always(lift_state(phase_i(controller_id))));
    ...
    entails_and_split(spec_a, sync_spec_with_desired(k, b, spec_ok, cluster, controller_id, janitor_id, outer), always(lift_state(phase_i(controller_id))));
    ...
    entails_and_split(spec_b, sync_spec_with_desired(k, b, spec_ok, cluster, controller_id, janitor_id, outer), always(lift_state(phase_i(controller_id))));
    ...
    let e1b = lift_state(Cluster::the_synced_object_in_reconcile_is(controller_id, outer));
...
pub open spec fn sync_step_ctx(k: SyncKind, b: Binding, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView) -> StatePred<ClusterState> {
    |s: ClusterState| {
...
pub open spec fn sync_step_next(k: SyncKind, b: Binding, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView) -> ActionPred<ClusterState> {
...
pub proof fn lemma_always_sync_step_next(k: SyncKind, b: Binding, spec_ok: spec_fn(Value) -> bool, spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int, janitor_id: int, outer: SyncedObjectView)
    requires
        ...
        sync_membership(k, b, spec_ok, cluster, controller_id, janitor_id),
        spec.entails(sync_spec_with_phase_ii(k, b, spec_ok, cluster, controller_id, janitor_id, outer)),
    ensures spec.entails(always(lift_action(sync_step_next(k, b, cluster, controller_id, janitor_id, outer)))),
{
    let key = outer.object_ref();
    ...
    temp_pred_equality(
        lift_state(sync_step_ctx(k, b, cluster, controller_id, janitor_id, outer)),
        lift_state(Cluster::each_object_in_etcd_is_weakly_well_formed())
    ...
    combine_spec_entails_always_n!(
        spec, lift_action(sync_step_next(k, b, cluster, controller_id, janitor_id, outer)),
