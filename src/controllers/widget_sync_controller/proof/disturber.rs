...
use crate::widget_sync_controller::{
    model::{disturber_reconciler::*, install::*},
...
pub proof fn lemma_always_widget_disturber_guarantee(spec: TempPred<ClusterState>, cluster: Cluster, controller_id: int)
    requires
        ...
        cluster.controller_models.contains_pair(controller_id, widget_disturber_controller_model()),
            ...
            if s.in_flight().contains(msg) {
                ...
                match step {
                    Step::ControllerStep(input) => {
                        ...
                        lemma_disturber_new_request_is_guaranteed(cluster, controller_id, s, s_prime, input, msg);
                    },
                    _ => { assert(false); },
                }
            }
    ...
    ensures disturber_request_is_guaranteed(msg),
{
    ...
    match state.reconcile_step {
        WidgetDisturberStepView::Init => {
            assert(req == APIRequest::PatchRequest(disturbing_patch(inner)));
            assert(disturbing_patch(inner).key() == cr_key);
        },
        WidgetDisturberStepView::AfterPatchInner => {
            assert(req == APIRequest::DeleteRequest(disturbing_delete(inner)));
        },
        _ => { assert(false); },
    }
}
...
pub proof fn disturber_guarantee_implies_relies(id: int)
    ensures
        lift_state(widget_disturber_guarantee(id)).entails(lift_state(widget_sync_rely(id))),
        lift_state(widget_disturber_guarantee(id)).entails(lift_state(widget_janitor_rely(id))),
{
    assert forall |s: ClusterState| #[trigger] widget_disturber_guarantee(id)(s) implies widget_sync_rely(id)(s) && widget_janitor_rely(id)(s) by {
